...
export default function App() {
  ...
  const renderRoleDashboard = () => {
    switch (user.role) {
      ...
      case 'Teacher': return <TeacherUpload />;
      case 'Student': return <StudentGrades />;
      default:
        return (
    ...
    <div className="min-h-screen bg-slate-950 relative font-sans text-slate-200">
      <header className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleLogout} className="text-xs font-mono text-red-400 hover:text-red-300 border border-red-900/40 hover:border-red-500 bg-red-950/20 px-3 py-1 rounded transition-all">
            DISCONNECT
          </button>
        </div>
      </header>

      <main className="py-6">{renderRoleDashboard()}</main>
